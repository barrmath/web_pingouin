...
def kmeans_bool(data_train, target_train ,data_test ,target_test,ax):
    ...
    ax = rapport_confusion(predict_test=predict_test_KMeans,target_test=target_test,ax = ax)
    
    return model_KMeans,ax


def KNNtest(data_train, target_train ,data_test ,target_test,ax,ax_knn,kmin=1,kmax=10):
  ...
  """
    ...
    ax = rapport_confusion(predict_test=predict_test_KNN,target_test=target_test,ax=ax)

    return model_voisin,ax,ax_knn


def regression_logistique(data_train, target_train ,data_test ,target_test,ax,max_iter=100):
    """
